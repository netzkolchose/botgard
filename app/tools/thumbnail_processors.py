...
# Note: Below is copied from easy_thumbnails.processors v2.10.1
#   FrameAware is adjusted to catch errors from self.im.seek(i)
#   because there are JPGs in the wild that have n_frames set to 2, but only have one frame :-(
    ...
    def apply_to_frames(self, method, *args, **kwargs):
        ...
        for i in range(self.im.n_frames):
            try:
                ...
                print(f"Exception in Image.seek({i}): {type(e).__name__}: {e}", file=sys.stderr, flush=True)
                continue
